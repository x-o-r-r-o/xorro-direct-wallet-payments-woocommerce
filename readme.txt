...
* Security (independent audit of this release): a non-exact transfer (partial, over or late) is credited only when it can belong to one order. On a shared address, a fee-short payment from one customer could otherwise be credited to a different, older unpaid order; such transfers are now left uncredited and the store owner gets one "unassigned payment to check" email. Cancelled orders and orders abandoned more than a day ago don't count as candidates
