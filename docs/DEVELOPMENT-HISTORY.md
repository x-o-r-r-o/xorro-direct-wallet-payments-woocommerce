...
| 1.25.0 | Vendors paid directly | `Xdwp_Wallets::pick_address()` became the one documented answer to "where should this money go?" — order-aware, with an `xdwp_receiving_address` filter and a guard that refuses any answer failing `is_plausible_address()` and falls back to the shop. `Xdwp_Vendors` hooks that filter only when a marketplace is detected *and* the merchant opted in; every marketplace call is behind `function_exists`/`method_exists`, so an upstream rename disables routing rather than breaking checkout. An order attributable to anything other than exactly one vendor pays the shop. Addresses are entered through WordPress core profile hooks, needing no marketplace-specific integration. `tests/vendor-tests.php` exists mostly to prove a single store is unchanged |
| 1.24.0 | WalletConnect, on a short leash | `Xdwp_Settings::walletconnect_project_id()` is the single definition of a usable id and is applied on read as well as on save — a settings option has more than one writer, and this value ends up on a page telling a customer where to send money. `Xdwp_Coins::WALLETCONNECT_SRC` pins the library version in code (filterable via `xdwp_walletconnect_src`), and `wallet.js` imports it inside the click handler, so a payment page nobody uses it on loads no third-party code at all. Pairing hands back an EIP-1193 provider and reuses `payWith()` rather than duplicating the transaction building. Email payment details also gained a wallet deep link in 1.23.1. `tests/walletconnect-tests.php` is mostly about restraint |
| 1.23.0 | Rehearsing the other half | `Xdwp_Rehearsal` creates one flagged order, quotes it through `Xdwp_Order::assign_payment()` and confirms it through the ordinary `mark_paid()`, so what fires is what fires for real. `confirm()` takes no order from its caller and re-checks `is_rehearsal()` before `mark_paid()`; `delete_order()` refuses anything unflagged and releases the reserved amount slot. Flagged orders are excluded from the payments query and from the report SQL (`rehearsal IS NULL`) so a dry run never reads as a sale. `tests/rehearsal-tests.php` attacks the guarantee from several directions |
| 1.22.0 | Rehearsal on test networks | `Xdwp_Testmode` plus three switch points, chosen so nothing else had to change: `Xdwp_Coins::get_payable()` restricts the coin list, `Xdwp_Wallets::is_plausible_address()` swaps the address shape *instead of* the mainnet one, and `find_payment_on_chain()` branches before any mainnet lookup — a chain with no test network returns false rather than reading mainnet. Testnet reading reuses the mainnet readers pointed elsewhere (Esplora testnet, Etherscan V2 chain 11155111, `check_tron_nile()`), because a rehearsal exercising different code would prove nothing. `tests/testmode-tests.php` covers both directions of the address rule |
| 1.21.0 | Money with no order | `Xdwp_Verifier` gains a listing capability written apart from the matching path and sharing none of its state: `can_list()`, `list_transfers()` and per-family listers for Esplora, Blockchair, Blockbook, Etherscan V2, Etherscan clones and Blockscout v2, with `raw_to_decimal()`/`add_digits()` doing the arithmetic as integer strings. `Xdwp_Reconcile` subtracts every txid the plugin can account for and reports the remainder, distinguishing *unreadable chain*, *explorer refused* and *nothing found* — conflating those would turn a partial look into a false all-clear. `tests/reconcile-tests.php` asserts the reconciler never writes to an order |
| 1.20.0 | An answer, not a spinner | `Xdwp_Order::payment_verdict()` reads the chain through the ordinary verification path and `verdict_for()` turns what it found into the sentence the customer reads — the two split so the wording can be tested without a chain, a database or an order (`tests/verdict-tests.php`, 53 assertions). The "I have sent it" endpoint takes an optional customer transaction id, recorded as testimony and never as proof. `Xdwp_Selftest` gains `check_outbound_allowed()` (WP_HTTP_BLOCK_EXTERNAL) and `check_loopback()`, which asks the site the same question a customer's browser asks. Eight accounting columns in the export, and `_xdwp_rate` recorded at quote time because it cannot be recovered later. From research across wordpress.org, CodeCanyon and the WooCommerce/WCAG specs |
| 1.19.9 | Configuration in parts | `Xdwp_Backup` gains scopes (all / wallets / keys): `payload()` filters by scope, a wallets file forces secrets off and a keys file forces them on, `filename()` names the download, and `restore()` reports which kind of file it read. The controls move into `includes/admin/views/backup-ui.php` and appear on General, Wallets and Prices & APIs. `Xdwp_Gateway::needs_setup()` and `get_transaction_url()` close two WooCommerce contract gaps found by auditing the gateway API — the second makes the order screen link a txid to its explorer. `tests/backup-tests.php` covers what each scope carries and, more importantly, what a partial restore must leave alone |
| 1.19.8 | One readme | `README.md` dropped from the release ZIP (asserted by the build and by a smoke check); PHP range and dual order-storage support stated plainly in the requirements and FAQ rather than implied |
